def hour_day_month_utc(data):
    ...
    """
    data["Hour"] = data["date"].str.extract(r"(\d{2}):\d{2}:\d{2}")
    data["Day"] = data["date"].str.extract(r"\d{4}-\d{2}-(\d{2})")
    data["Month"] = data["date"].str.extract(r"\d{4}-(\d{2})-\d{2}")
    data["UTC"] = (
        data["date"]
        .str.extract(r"\b(GMT| -0000)\b")
        .replace(["GMT", " -0000"], "+0000")
    )
...
def weekend_flag(data):
    """Create weekend flag based on date.
    ...
    -------
    dataframe
        Dataframe with new boolean column: 'weekend'.
    """
    weekend_days = ["Sat,", "Sun,"]
    data["weekend"] = data["date"].str.split().str[0].isin(weekend_days).astype(int)
